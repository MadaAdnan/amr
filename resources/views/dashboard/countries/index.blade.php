    ...
    <div class="page-title">
        ...
        <div class="card">
            <div class="row" id="basic-table">
                <div class="col-12">
                    <div class="row p-4">
                        <h4 class="w-50 font-weight-bold">Locations</h4>
                        <div class="w-50 text-right">
                            <button onclick="showCreateModal()" type="button"
                                class="btn btn-primary remove-border w-50">Add</button>
                        </div>
                    </div>
                                    ...
                                    <tbody>
                                        @foreach ($data as $item)
                                            <tr>
                                                ...
                                                <td class="text-bold-500 text-center"><span
                                                        style="background-color: {{ $status[$item->status]['color'] }}; !important"
                                                        class="badge badge-success {{ $item->status == 'publish' ? 'success-badge' : 'danger-badge' }} text-uppercase">{{ $item->status == 'publish' && $item->date && $item->date->gt($nowDate) ? $status['Scheduled']['name'] : $status[$item->status]['name'] }}</span>
                                                </td>
                                                <td class="text-bold-500 text-center">
                                                    {{-- <button onclick="openEditModal('{{ $item }}')" class="btn btn-dark text-white btn-sm custom-button"><i data-feather="edit" width="20"></i></button> --}}
                                                    <a href="{{ route('dashboard.countries.states_index', $item->id) }}"
                                                        class="btn btn-primary btn-sm custom-button"><i data-feather="eye"
                                                            width="20"></i></a>
                                                </td>
                                            </tr>
                                        @endforeach
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="createModalLabel">Add a Country</h5>
                                </div>
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button onclick="createButton()" type="button" class="btn btn-success">Add</button>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <div class="form-group mb-3">
                                        ...
                                        <input disabled id="editNameInput" type="text" class="form-control"
                                            placeholder="please add country name" aria-label="please add country name"
                                            aria-describedby="basic-addon2">
                                    </div>
                                    <div class="form-group">
                                        <label for="" class="mb-2">Status</label>
                                        <select id="editStatusInput" class="form-select"
                ...
                @section('scripts')
                    <script>
                        ...
                        const nameInput = document.getElementById('selectCountry');
                        ...
                        function filter() {
                            ...
                            let url = '{{ route('dashboard.countries.index') }}'
                        ...
                        function createButton() {
                            let request = '{{ route('dashboard.countries.store') }}';
                            if (!valdation()) return;
                            $.ajax({
                                url: request,
                                type: 'POST',
                        ...
                        function updateButton() {
                            let request = '{{ route('dashboard.countries.update', ':id') }}';
