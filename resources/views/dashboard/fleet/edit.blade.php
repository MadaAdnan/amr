...
<style>
    ...
    <div class="card p-3">
        ...
        <div class="alert alert-danger p-5" role="alert">
         ...
         @endif
        <form enctype="multipart/form-data" action="{{ route('dashboard.fleets.update',$data->id) }}" id="AddForm" method="post">
            @csrf
            ...
            <div class="mb-3">
             ...
             <div class="mb-3">
              ...
              <div class="mb-3">
                ...
                <textarea id="category_description"  name="content" class="form-control">{{ $data->content }}</textarea>
              </div>
              <div class="mb-3">
                <label for="license" class="form-label">License</label>
                <input value="{{ isset($data)?$data->license:''}}" name="license" type="text" class="form-control" id="license" required />
              </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="categories" id="categories" class="form-control">
                            @foreach ($categories as $item)
                                <option {{ $item->id == $data->category->id?'selected':'' }} value="{{ $item->id }}">{{ $item->title }}</option>                                
                            @endforeach
                        </select>
                      </div>
                </div>
