<script>
    ...
    let flag = false;
    var selectedChildSeat = [];
    let selectedAmount = [];

    let pickUplocationObj = {};
    let dropOffLocationObj = {};
    let fleet = null;
    let distance = null;
    let map = null
    let isAirport = false;
    ...
    var storedCountryCode = '{{ isset($reservation->users) ? $reservation->users->country_code : '' }}';
    ...
    if (isEditPage == true) {
        const reservation_ = '{{ isset($reservation) ? json_encode($reservation) : null }}';
        const decodedReservationString = new DOMParser().parseFromString(reservation_, 'text/html').body.textContent;

        // Parse JSON
        reservation = JSON.parse(decodedReservationString);

        var bookedChildSeats = {!! isset($selectedChildSeats) ? json_encode($selectedChildSeats) : 'null' !!}

        // Populate the booked child seats in the modal
        for (let index = 0; index < bookedChildSeats.length; index++) {
            const bookedSeat = bookedChildSeats[index];

            // Access child seat name, title, and amount from the pivot table
            const childSeatName = bookedSeat.title;
            const childSeatId = bookedSeat.pivot.child_seats_id;
            const childSeatAmount = bookedSeat.pivot.amount;

            // Store the information in the selectedChildSeat array as an object
            selectedChildSeat.push({
                name: childSeatName,
                id: childSeatId,
                amount: childSeatAmount
            });
        }
    ...
    // Function to convert numeric country code to ISO 2 code
    function getIso2Code(numericCode) {
        var countryData = window.intlTelInputGlobals.getCountryData();
        var iso2Code = '';

        for (var i = 0; i < countryData.length; i++) {
            if (countryData[i].dialCode === numericCode) {
                iso2Code = countryData[i].iso2;
                break;
            }
        }
    ...
    var iso2Code = getIso2Code(storedCountryCode);

    var iti = window.intlTelInput(input, {
        initialCountry: iso2Code,
    ...
    flagElement.className = "iti__flag iti__" + selectedCountryData.iso2;
    ...
    function editChildSeatsModal() {
        ...
        if (isEditPage == true) {
            ...
            for (let i = 0; i < selectedChildSeat.length; i++) {
                const seat = selectedChildSeat[i];

                // Generate HTML for the seat type (child seat)
                let seatSelect = `<select id="${containerId}" onchange="getPriceAccordingToFleet()" class="form-control select2" name="child_seats[][seats]">`;
                // let seatSelect = `<select id="${containerId}" class="form-control" name="child_seats[][seats]" onchange="getPriceAccordingToFleet()">`;
                for (let j = 0; j < child_seats.length; j++) {
                    const childSeat = child_seats[j];
                    const selected = seat.id === childSeat.id ? "selected" : "";
                    seatSelect += `<option value="${childSeat.id}" data-price="${childSeat.price}" ${selected}  >${childSeat.title}</option>`;
                }
                seatSelect += `</select>`;

                // Generate HTML for the amount
                let amountSelect = `<select id="${idDivAmount}" onchange="checkChildSeatsNumber()" class="form-control select2" name="child_seats[][amount]">`;
                for (let amount = 1; amount <= 3; amount++) {
                    const selected = seat.amount == amount ? "selected" : "";
                    amountSelect += `<option value="${amount}" ${selected}>${amount}</option>`;
                }
                amountSelect += `</select>`;

                // Create a container for the pair of select elements
                let container = `<div class="row mt-3" id= ${i}>
                                    <div class="col-6">
                                    ...
                                    <div class="text-right">
                                        <button onclick="deleteChildSeat(${i})" class="mt-2 btn btn-danger btn-sm"> <i class="fa fa-trash"></i> </button>                    
                                     </div>
                                 </div>
                `;

                // Append the container to a target element (e.g., '#seatsSections')
                $('#seatsSections').append(container);
            }
        }
        // getChildSeats();
    ...
    function getPriceAccordingToFleet() {
        ...
        const data = {
            ...
            distance: distance ?? milesInput.value,
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (changeCounter <= 0) {
                    ...
                    else if(enablePriceCehckBox.checked && reservation)
                    {
                    ...
                    else 
                    {
                        //*******************************************
                        // It includes : 
                        //  1- enaibled && exist Reservation
                        //  2- !enaibled && !exist Reservation
                        //  3- enaibled && !exist Reservation
                        //*******************************************
                        if (reservation) 
                        {
                            // 1- enaibled && exist Reservation
