...
@endphp
    <form enctype="multipart/form-data" action="{{ route('dashboard.pages.update',$data->id) }}" id="AddForm" method="post">
        @csrf
            <div class="row">
                ...
                <div class="col-3">
                    ...
                    @if (($data->status == 'Rejected'||$data->status == 'Pending'||$data->status == 'Published'||$data->status == 'Draft'))
                        <div class="card p-3">
                                <button  {{ $data->status == 'Published' && $role != 'Seo-admin'?'disabled':'' }} type="button" onclick="publish()" class="btn btn-success">
                                    {{ $data->status == 'Published'?'Update':'Submit' }}
                                </button>                            
                        </div>
                    @endif
                    ...
                    @if ($data->status == 'Rejected'||$data->status == 'Pending'||$data->status == 'Published'||$data->status == 'Draft')
                        <div class="card p-3">
                            @if (($data->status !== 'Published'&&$role != 'Seo-specialist')||($data->status == 'Draft'&&$role = 'Seo-specialist')||($data->status == 'Rejected'&&$role = 'Seo-specialist'))                            
                                <div class="col text-center d-flex">
                                    <button disabled onclick="sendPreviewRequest()" class="btn btn-secondary m-2 w-100">Preview</button>
                                        <button onclick="sendSaveRequest()" type="submit" class="btn btn-primary m-2">Save</button>                                    
                                </div>
                                @else 
                                <button disabled onclick="sendPreviewRequest()" class="btn btn-secondary">Preview</button>
                            @endif
                        </div>
                    @endif
                    ...
                    <div class="card p-3">
                        <div class="form-group">
                            ...
                            <input value="{{ $data->seo_title }}" maxlength="70" onkeydown="countWord(event,'maxCharSeoTitle')" class="form-control" name="seo_title" />
                        </div>
                    ...
                    <div class="card p-3">
                        <div class="form-group">
                            ...
                            <textarea maxlength="150" onkeydown="countWord(event,'maxCharSeoDescription')" id="seo_description" name="seo_description"  class="form-control">{{ $data->seo_description }}</textarea>
                        </div>
                    ...
                    <div class="card p-3">
                        ...
                        <select id="navPageSelect" class="form-control select2" name="navPage">
                            @foreach ($navPages as $item)
                                <option  {{ $item->id == $data->navSections->id?'selected':'' }} value="{{ $item->id }}">{{ $item->title }}</option>
                            @endforeach
                        </select>
                    </div>
...
@section('scripts')
    <script>
            ...
            $("#content").val(`{!! $data->content !!}`);
              ...
              function publish()
            {
                const request = '{{ route("dashboard.pages.store_with_publish",$data->id) }}';
            ...
            {
                document.getElementById('AddForm').action = '{{ route("dashboard.pages.preview",$data->id) }}'
                $('#AddForm').submit();
            }
            function sendSaveRequest()
            {
                document.getElementById('AddForm').action = "{{ route('dashboard.pages.update',$data->id) }}";
                $('#AddForm').submit();
            }
