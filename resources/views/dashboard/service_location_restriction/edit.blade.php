...
@section('content')
    ...
    <div class="card p-3">
        ...
        @endif
        <form enctype="multipart/form-data" action="{{ route('dashboard.serviceLocationRestrictions.update',$data->id) }}" id="AddForm" method="post">
                @csrf
            <h3>
                Edit 
            </h3>
            ...
            <div class="row">
                    ...
                    <div class="col-6">
                        ...
                        <div class="row">
                            <div class="col-10">
                                <input value="{{ $data->address }}" {{ !$data->city_id ? 'disabled':''  }} required name="address" type="text" class="form-control" id="address" placeholder="Enter place name">
                            </div>
                            <div class="col-2">
                                <button id="search_button" {{ !$data->address ?'disabled':'' }} onclick="performSearch()" type="button" class="btn btn-primary btn-sm">
                    ...
                    <div class="col-6">
                        ...
                        <div class="row">
                            <div class="col">
                                <input min="0" value="{{ $data->radius }}" onkeyup="updateRadius()" id="radius" {{ !$data->city_id ? 'disabled':''  }} required name="radius" type="number" class="form-control" id="radius" placeholder="Restriction radius">
                            </div>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="mb-3">
                            ...
                            <select name="service" class="form-control" id="service">
                                <option  {{ $data->service == 'both' ? 'selected':'' }} value="both">Both</option>
                                <option  {{ $data->service == 'point_to_point' ? 'selected':'' }} value="point_to_point">Point to point</option>
                                <option  {{ $data->service == 'hourly' ? 'selected':'' }} value="hourly">Hourly</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="mb-3">
                            ...
                            <select name="service_limitation" class="form-control" id="service_limitation">
                                <option  {{ $data->service_limitation == 'both' ? 'selected' : '' }} value="both">Both</option>
                                <option  {{ $data->service_limitation == 'pick_up' ? 'selected' : '' }} value="pick_up">Pick up</option>
                                <option  {{ $data->service_limitation == 'drop_off' ? 'selected' : '' }} value="drop_off">Drop off</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-6">
                        <div class="mb-3">
                            ...
                            <select name="status" class="form-control" id="service_limitation">
                                <option  {{ $data->service_limitation == 'active' ? 'selected' : '' }} value="active">Active</option>
                                <option  {{ $data->service_limitation == 'inactive' ? 'selected' : '' }} value="inactive">Inactive</option>
                            </select>
                        </div>
                    </div>
            </div>
                ...
                <div class="row mt-5">
                    ...
                    <div class="col-6">
                        <div class="mb-3">
                            ...
                            <input  value="{{ $data->latitude }}" required disabled name="latitude" type="text" class="form-control" id="latitude">
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="mb-3">
                            <label for="long" class="form-label">Long</label>
                            <input  value="{{ $data->longitude }}" required disabled name="longitude" type="text" class="form-control" id="longitude">
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        $("#AddForm").validate({
            ...
            submitHandler: function(form) {
                ...
                let request =  "{{ route('dashboard.serviceLocationRestrictions.check_if_place_available',[':lat',':long',':radius',':id']) }}";
                ...
                request = request.replace(':id',restrictionId);
                ...
                $.ajax({
                        ...
                        error: function (xhr, status, error) {
                            // Handle the error of the pre-submission request
        ...
        initMap();
        selectPlace(latitudeInput.value,longitudeInput.value,searchText.value)
        updateRadius()
