                ...
                <div class="col-12">
                    <div class="row p-4">
                        ...
                        <div class="w-50 text-right">
                            <a href="{{ route('dashboard.companies.create') }}" type="button"
                                class="btn btn-primary remove-border w-50">Create</a>
                        </div>
                    </div>
                                    ...
                                    <tbody>
                                        @foreach ($companies as $item)
                                            <tr>
                                                ...
                                                <td class="text-bold-500 text-center">{{ $item->company_name ?? 'No Data' }}
                                                </td>
                                                <td class="text-bold-500 text-center">{{ $item->email ?? 'No Data' }}</td>
                                                <td class="text-bold-500 text-center">{{ $item->phone }}</td>
                                                <td class="text-bold-500 text-center">
                                                    {{ $item->state ? $item->state->name : 'No Data' }}</td>
                                                <td class="text-bold-500 text-center">
                                                    {{ $item->country ? $item->country->name : 'No Data' }}</td>
                                                <td class="text-bold-500 text-center">
                                                    {{ $item->city ? $item->city->title : 'No Data' }}</td>

                                                <td class="text-bold-500 text-center">{{ $item->street }}</td>
                                                <td class="text-bold-500">
                                                    @if ($pageType != 'trashed')
                                                        <a href="{{ route('dashboard.companies.softDelete', $item->id) }}"
                                                            class="btn btn-danger text-white btn-sm custom-button "> <i
                                                                data-feather="trash" width="20"></i> </a>
                                                        <a href="{{ route('dashboard.companies.edit', $item->id) }}"
                                                    ...
                                                    @if ($pageType === 'trashed')
                                                        ...
                                                        <a data-id={{ $item->id }}
                                                            ...
                                                            href="{{ route('dashboard.companies.softDeleteRestore', $item->id) }}">
